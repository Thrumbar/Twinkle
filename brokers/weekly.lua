...
OnLDBEnter = function(self)
	...
	lineNum = tooltip:AddHeader()
			  tooltip:SetCell(lineNum, 1, addonName .. ': ' .. _G.CALENDAR_REPEAT_WEEKLY, 'LEFT', numColumns)
...
ns.RegisterEvent('ADDON_LOADED', function(frame, event, arg1)
	if arg1 == addonName then
		LDB:NewDataObject(addonName..moduleName, {
			type	= 'data source',
			label	= _G.CALENDAR_REPEAT_WEEKLY,
			-- text 	= CURRENCY,
		...
		ns.UnregisterEvent('ADDON_LOADED', 'weekly')
	end
end, 'weekly')
